...
module.exports = {
  ...
  execute(message, args) {
    ...
    if (!message.guild.me.hasPermission(["MANAGE_CHANNELS"]))
      return message.channel.send(
        "I don't have enough permissions to manage channels. [`MANAGE_CHANNELS`]"
      );
    ...
    if (message.member.hasPermission(["MANAGE_CHANNELS"])) {
      ...
      if (amount > 21600)
        return message.channel.send(
          `The maximum input for this command is 21600.`
        );
      message.channel.setRateLimitPerUser(amount);
      message.channel.send(`Current cooldown for this channel : ${amount}s.`);
    } else
      message.channel.send("You need `MANAGE_CHANNELS` to use this command");
  },
};
