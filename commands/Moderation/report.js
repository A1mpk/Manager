...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!data) {
      ...
      if (!result)
        return message.channel.send(
          `The report channel was either deleted or I don't have access to it.`
        );
    ...
    collector.on("end", async (collected) => {
      message.channel.send(
        `Your report has been sent to the Admins. Expect a reply soon but do not forget to keep your dms on so Admins can talk to you privatly.`
      );
      let counter = 0;
      ...
      const ReportChannelEmbed = new Discord.MessageEmbed()
        .setAuthor(
          `Report by ${message.member.user.tag}`,
          message.member.user.displayAvatarURL()
        )
        .addField(questions[counter++], collected.first().content)
        .addField(questions[counter++], collected.last().content)
        .setDescription(
          `To take care of this case, you must DM the user or talk to them on a private channel. To trash a report, you could react with ❌.`
        )
      ...
      const collector2 =
        ReportChannelEmbed2.createReactionCollector(reactionFilter);
