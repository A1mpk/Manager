...
module.exports = {
  ...
  execute(message, args) {
    ...
    if (message.member.hasPermission("MANAGE_MESSAGES")) {
      ...
      if (amount > 100)
        return message.channel.send(`Please enter an number below **100**.`);
      ...
      message.channel
        .send(`I have cleared **${amount}** messages!`)
        .then((m) => m.delete({ timeout: 5000 }));
    } else
      message.channel.send("You need `MANAGE_MESSAGES` to use this command.");
  },
};
