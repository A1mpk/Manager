...
module.exports = {
  ...
  execute(message, args) {
    ...
    if (!message.guild.me.hasPermission(["MANAGE_NICKNAMES"]))
      return message.channel.send(
        "I don't have enough permissions to ban a user. [`MANAGE_NICKNAMES`]"
      );
    ...
    if (message.member === message.guild.owner) {
      ...
      if (lol) {
        if (lol.length > 32)
          return message.channel.send(
            `Your nickname has more than 32characters, 32 is the max!`
          );
        if (lol.length < 1) {
          message.channel.send(
            `You do not have enough of characters to set a nickname.`
          );
        } else message.member.setNickname(lol);
        if (message.member === message.guild.owner) return;
        message.channel.send(
          `Your nickname on this server has been changed to **${lol}.**`
        );
      }
    } else
      message.channel.send("You need `CHANGE_NICKNAME` to use this command.");
  },
};
