  ...
  if(!Arguments)return message.channel.send(`You need to enter the channel name you want me to log the reports🧍‍♂️`)

  const ChannelIdentifier = message.guild.channels.cache.find(Channel => Arguments === Channel.name)
 if(!ChannelIdentifier)return message.channel.send(`I can't find that channel, make sure you've copy pasted the exact name of the channel.`)
 ...
 try {
   ...
   try {
     ...
     message.channel.send(`You will now get new reports in ${ChannelIdentifier}`)
   }catch(er){
