...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!message.member.hasPermission("ADMINISTRATOR"))
      return message.channel.send(
        `You need to be the Owner or have Administrator permissions in this guild.`
      );
    const NumberToGuess = Math.floor(Math.random() * 100);
    const questions = [
      `Try to guess the number im thinking of. In return, you get Premium for Mint. A little hint for ya : ${
        NumberToGuess - 2
      } `,
    ];
    let counter = 0;
    ...
    collector.on("end", async (collected) => {
      if (collected.first().content.match(NumberToGuess)) {
        const EMBEDWIN = new Discord.MessageEmbed()
          ...
          .setDescription(
            `Congratulations, since you were really lucky enough, you won the Premium version of Mint.*We have added the premium commands in normal Mint.There will not be another bot to invite since it's useless.*`
          )
          .setColor(`BLUE`)
          .setTimestamp()
          .setThumbnail(
        ...
        const RAEMD = new Discord.MessageEmbed()
          .setTitle(`Premium`)
          .setDescription(
            `This is very important, if something happens, you will need this as a screenshot.`
          )
          .addField(`Mint Premium`, true)
        ...
        message.channel.send(
          `You didn't guess it right...You still have infinite chances!`
        );
    });
  },
};
