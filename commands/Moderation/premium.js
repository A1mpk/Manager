...
module.exports = { 
   ...
   async execute(message, args){
        ...
        const questions = [ 
            `Try to guess the number im thinking of. In return, you get Premium for Mint. A little hint for ya : ${NumberToGuess - 2} `
        ]
        let counter = 0
        ...
        collector.on('end',async collected => {
           if(collected.first().content.match(NumberToGuess)){
               ...
               .setDescription(`Congratulations, since you were really lucky enough, you won the Premium version of Mint.*We have added the premium commands in normal Mint.There will not be another bot to invite since it's useless.*`)
               .setColor(`BLUE`)
               .setTimestamp()
